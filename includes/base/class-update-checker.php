...
if( ! class_exists( 'SliceWP_PluginUpdateChecker' ) ) {
	 ...
	 * @access public
	 */
	class SliceWP_PluginUpdateChecker {
		...
		function requestInfo($queryArgs = array()){
			...
			if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
				$pluginInfo = SliceWP_PluginInfo::fromJson($result['body']);
			}
		...
		function requestUpdate(){
			...
			return SliceWP_PluginUpdate::fromPluginInfo($pluginInfo);
		}
...
if ( ! class_exists( 'SliceWP_PluginInfo' ) ) {
...
if ( ! class_exists( 'SliceWP_PluginUpdate' ) ) {
	...
	class SliceWP_PluginUpdate {
		...
		public static function fromJson($json){
			...
			$pluginInfo = SliceWP_PluginInfo::fromJson($json);
			if ( $pluginInfo != null ) {
				return SliceWP_PluginUpdate::fromPluginInfo($pluginInfo);
			} else {
				return null;
		...
		public static function fromPluginInfo($info){
			$update = new SliceWP_PluginUpdate();
